...
public class HateoasFormation {
	...
	private Link generateHateoas(HttpServletRequest request, String linkType, int totPages, int page, int size) {
		Map<String, String[]> queryParams = request.getParameterMap();
		int curPageNo = page;
		if((linkType.equals("CUR")) || (linkType.equals("NEXT") && curPageNo+1 == totPages) || (linkType.equals("PREV") && curPageNo == 0)) {
			if(request.getQueryString() != null) {
				return new Link(request.getRequestURL().toString()+"?"+request.getQueryString(), linkType, request.getMethod());
			} else {
				return new Link(request.getRequestURL().toString()+"?page="+curPageNo+"&size="+size, linkType, request.getMethod());
			}
